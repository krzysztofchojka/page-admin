...
class UserController {
    ...
    public function update() {
        ...
        if (!empty($pass)) {
            ...
            $db->query("UPDATE pa_users SET uname = ?, email = ?, pass = ?, admin = ?, pass_expired = ? WHERE id = ?", [$uname, $email, $hash, $isAdmin, $forceChange, $id]);
        } else {
            $db->query("UPDATE pa_users SET uname = ?, email = ?, admin = ?, pass_expired = ? WHERE id = ?", [$uname, $email, $isAdmin, $forceChange, $id]);
        }
