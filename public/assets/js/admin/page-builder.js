...
const blockIcons = {
    text: 'T', image: '🖼', video: '▶️', button: '🔘', divider: '➖',
    quote: '❝', columns_2: '◫', columns_3: '☰', banner: '🏔',
    image_cards: '🗂', carousel: '🎠', accordion: '⇕', map: '📍',
    countdown: '⏳', table: '🗄️', flight: '✈️', form: '📝',
    gallery: '📷', raw_html: '</>'
};

const blockNames = {
    text: 'Tekst', image: 'Obrazek', video: 'Wideo', button: 'Przycisk', divider: 'Odstęp',
    quote: 'Cytat', columns_2: '2 Kolumny', columns_3: '3 Kolumny', banner: 'Baner',
    image_cards: 'Siatka Kart', carousel: 'Karuzela', accordion: 'Akordeon', map: 'Mapa',
    countdown: 'Odliczanie', table: 'Tabela', flight: 'Loty', form: 'Formularz',
    gallery: 'Galeria', raw_html: 'HTML'
};
...
// ==========================================
// 2. DEFINICJE FUNKCJI (CORE & HELPERS)
// ==========================================

function updateNavigator() {
    const navTree = document.getElementById('navigator-tree');
    navTree.innerHTML = '';
    buildNavTree(document.getElementById('editor'), navTree);
    initNavSortables();
}
// Pozostawiamy podpięcie pod window dla kompatybilności z onClick w HTML
    ...
    document.querySelectorAll('.nav-drop-zone').forEach(el => {
        navSortables.push(Sortable.create(el, {
            ...
            onEnd: function (evt) {
                ...
                if (refZoneUid === 'editor') {
                    realZone = document.getElementById('editor');
                } else {
...
function getBlocksFromContainer(container) {
    ...
    Array.from(container.children).forEach(el => {
        ...
        if(type === 'columns_2') {
            ...
            content = { html: el.querySelector('.flight-html').value, state: JSON.parse(decodeURIComponent(el.querySelector('.flight-state').value || '%5B%5D')) };
        } else if (type === 'image_cards') {
            const cards = [];
...
function syncFlightVisualToRaw(block) {
    ...
    block.querySelectorAll('.flight-segment').forEach(seg => {
        ...
        if (type === 'flight') {
            const s = {
                type: 'flight', airline: seg.querySelector('.f-airline').value, logo: seg.querySelector('.f-logo').value,
                durPl: seg.querySelector('.f-dur-pl').value, durEn: seg.querySelector('.f-dur-en').value,
                depPl: seg.querySelector('.f-dep-pl').value, depEn: seg.querySelector('.f-dep-en').value,
                arrPl: seg.querySelector('.f-arr-pl').value, arrEn: seg.querySelector('.f-arr-en').value,
            };
            state.push(s);
...
// Poświęcamy osobny blok na wystartowanie skryptu, kiedy już wszystko powyżej istnieje.
